...
public class Queen extends ChessPiece {
	Queen(boolean whichplayer, int[] startingcoordinates, int numbergiven) {
			super(whichplayer, startingcoordinates, numbergiven, "queen");
		}
	...
	public ArrayList<int[]> simplifiedpossiblemoves(ChessPiece[][] Board) {
		...
		for (int i = 1; proceed && getposition()[0] + i < 8 && getposition()[1] + i < 8; i++) {
			if (Board[getposition()[0] + i][getposition()[1] + i].getdesignationnumber() == -1) {
				...
				output.add(new int[] { getposition()[0] + i, getposition()[1] + i,
						Board[getposition()[0] + i][getposition()[1] + i].getdesignationnumber() });
				proceed = false;
			} else {
				// piece at Board[getposition()[0]+i][getposition()[1]+i] is same side as the
				// bishop
